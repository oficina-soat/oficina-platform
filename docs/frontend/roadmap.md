...
- [ ] `[UI-FUT-USERS-TEST-001]` Cobrir contratos, autorização visual, idempotência, consistência eventual, aplicação, adapters, acessibilidade, falha parcial e fluxos E2E administrativos; homologar no `lab` com dados sentinela e investigar previamente a DLQ de `usuarioAdicionado`. Cobertura automatizada concluída localmente. No `lab`, o OS já confirmou criação, filtros remotos e `acoesPermitidas`; a autenticação, ativação, login e transições de estado já foram validadas em rodada anterior. A rodada final aguarda o deploy do Auth 1.3.1, que publica no API Gateway a rota administrativa `GET /auth/usuarios/{usuarioId}/credencial`, para repetir todo o percurso ponta a ponta com um único sentinela.
