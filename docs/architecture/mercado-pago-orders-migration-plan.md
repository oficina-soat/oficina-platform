...
O `lab` executa o Billing `1.10.3` no modo `orders`. A versão anterior `1.8.0` criava cobranças em `POST /v1/payments`, persistia o ID numérico do payment em `transacaoExternaId` e reconciliava por `GET /v1/payments/{id}`; essa integração apresentou corretamente o PIX, mas a cobrança sandbox permaneceu pendente na [homologação parcial](../delivery/payment-checkout-continuity-lab-evidence.md).

A versão `1.9.0` e a projeção de infraestrutura implementaram as quatro etapas preparatórias deste plano. A [evidência local da migração Orders](../delivery/payment-orders-local-evidence.md) registra contratos, migration, testes, cobertura e validações. A [homologação no lab](../delivery/payment-checkout-continuity-lab-evidence.md) comprovou criação e consulta de order, `APRO`, configuração do evento **Order (Mercado Pago)** e observabilidade. O Billing `1.10.1` corrigiu a unidade usada no cálculo da idade sem alterar o manifesto HMAC nem o contrato público; o `1.10.2` classificou um callback real histórico como `hash_mismatch`; e o `1.10.3`, já publicado e implantado, reconhece com `200` referências assinadas desconhecidas. O payload fictício do teste de URL do painel passou pela validação HMAC com a configuração atual e foi confirmado com `HTTP 200`; a etapa remota `[D-PAYMENT-CONTINUITY-TEST-REM-001]` permanece aberta para repetir o webhook real de Order até a entrega.
