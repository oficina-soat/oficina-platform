...
- [ ] `[D-NR-REM-005]` Executar teste de ponta a ponta no ambiente `lab` gerando uma Ordem de Serviço com caminho feliz e uma falha compensada, confirmando correlação por `correlationId` entre logs, traces, métricas e eventos. Execução registrada em 2026-07-11 no [Relatório D-NR-REM-005 — E2E no ambiente lab](docs/d-nr-rem-005-e2e-lab-report.md): o fluxo REST passou e métricas foram comprovadas em `Metric`, mas a etapa permanece pendente porque `Span`, logs com `correlationId` e logs/eventos com `eventType` continuaram sem evidência no New Relic. Correção local preparada em 2026-07-11 nos três microsserviços para traces, logs HTTP e logs estruturados de Outbox/eventos; falta publicar/deployar as novas versões e reexecutar a validação remota.
