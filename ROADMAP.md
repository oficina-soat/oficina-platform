...
- [ ] `[D-JOURNEY-FRESHNESS-BILLING-IDEMPOTENCY-REM-001]` Implantar o Billing `1.7.1` no `lab` e repetir a finalização concorrente pelos eventos `execucaoFinalizada` e `ordemDeServicoFinalizada`; comprovar uma única linha de pagamento, uma única Outbox, ACK dos dois eventos sem violação de chave, retry, nova DLQ ou regressão da jornada.
