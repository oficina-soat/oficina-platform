...
Uma revisão posterior identificou que a instrução manual do Mercado Pago normaliza `data.id` alfanumérico para minúsculas, enquanto o SDK Java oficial preserva o valor literal. O Billing `1.10.11` foi preparado localmente para validar ambas as formas exclusivamente com o mesmo secret e preservar o identificador original para consultar a Order. A cobertura inclui vetores HMAC fixos calculados independentemente, timestamp em milissegundos, identificadores alfanuméricos e não alfanuméricos, preservação entre query, validador e reconciliação, além das rejeições existentes. O `clean verify` passou com 208 testes, PostgreSQL 16, 10 migrations, arquitetura e JaCoCo; essa candidata ainda depende de Quality Gate, publicação, deploy e nova jornada real.

O [PR 48 do Billing](https://github.com/oficina-soat/oficina-billing-service/pull/48) passou no Quality Gate após indisponibilidades transitórias `500` e `504` do SonarCloud. O workflow da `main` publicou e implantou `1.10.11`, com rollout completo e uma réplica pronta. Na jornada executada após o deploy, o callback real chegou ao pod, mas a validação permaneceu em `hash_mismatch`: query e corpo continham o mesmo `data.id`, o identificador era alfanumérico em maiúsculas com 32 caracteres, a assinatura continha `ts,v1`, o manifesto incluía o ID e nenhuma variante alternativa correspondeu. Como o `1.10.11` tenta primeiro o ID alfanumérico em minúsculas e depois o literal, a caixa do identificador foi descartada como causa isolada sem flexibilizar o HMAC.
