...
- [ ] `[UI-OS-ITEMS-REM-001]` Implantar e homologar no `lab` a composição de uma OS com serviço e peça, comprovando orçamento derivado, reflexo de estoque, eventos, retries seguros e correlação, sem registrar dados pessoais ou financeiros sensíveis. Rotas, composição real, retries, conflito idempotente, ausência de movimentação prematura de estoque e tela publicada foram aprovados em 2026-07-16. O fechamento permanece pendente porque a OS está em `EM_DIAGNOSTICO`, mas sua execução vinculada já está em `REPARO_CONCLUIDO`; detalhes estão na [evidência do MVP no lab](ui-mvp-lab-evidence.md).
...
- [ ] `[UI-MVP-REM-001]` Homologar no `lab` os três fluxos do MVP com os papéis reais e registrar evidências de segurança, acessibilidade, pipeline e operação. Homologação iniciada em 2026-07-16; publicação, acesso público, configuração de runtime, fallback da SPA, health check, headers de segurança, gates automatizados e a composição renderizada no navegador real estão registrados na [evidência parcial do MVP no lab](ui-mvp-lab-evidence.md). Restam a jornada coerente até orçamento e pagamento, a execução com papel dedicado e a correlação da telemetria; o runtime publicado ainda não possui `observability` configurada.
