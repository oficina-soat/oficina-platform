...
O `lab` executa o Billing `1.10.7` no modo `orders`. A versão anterior `1.8.0` criava cobranças em `POST /v1/payments`, persistia o ID numérico do payment em `transacaoExternaId` e reconciliava por `GET /v1/payments/{id}`; essa integração apresentou corretamente o PIX, mas a cobrança sandbox permaneceu pendente na [homologação parcial](../delivery/payment-checkout-continuity-lab-evidence.md).

A versão `1.9.0` e a projeção de infraestrutura implementaram as quatro etapas preparatórias deste plano. A [evidência local da migração Orders](../delivery/payment-orders-local-evidence.md) registra contratos, migration, testes, cobertura e validações. A [homologação no lab](../delivery/payment-checkout-continuity-lab-evidence.md) comprovou criação e consulta de order, `APRO`, configuração do evento **Order (Mercado Pago)** e observabilidade. O Billing `1.10.1` corrigiu a unidade usada no cálculo da idade, o `1.10.2` adicionou diagnóstico sanitizado e o `1.10.3` passou a reconhecer referências assinadas desconhecidas. As versões `1.10.4` a `1.10.7` isolaram a origem do `data.id`, preservaram sua capitalização conforme o SDK oficial e aceitaram fallback pelo corpo quando a query não chega ao runtime. O payload fictício do painel e probes com o secret projetado recebem `HTTP 200`, mas callbacks reais de Orders continuam recebendo `401 hash_mismatch`; a etapa remota `[D-PAYMENT-CONTINUITY-TEST-REM-001]` permanece aberta para alinhar o secret de Webhooks do modo produção da aplicação e repetir o fluxo até a entrega.
