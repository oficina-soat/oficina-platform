...
- o Billing `1.10.1` persiste as instruções, cria e consulta Orders, converge atualização manual por transição condicional e Outbox determinística, não permite confirmação manual de cobrança integrada e normaliza o `ts` de 13 dígitos somente ao verificar a tolerância do webhook;
...
A [homologação da continuidade do pagamento no lab](../delivery/payment-checkout-continuity-lab-evidence.md) já comprovou implantação, Quality Gates, apresentação do PIX, reconciliação server-to-server, duplicidade, ordem, concorrência, unicidade e sanitização remota. A jornada com Orders também comprovou `APRO`, order `processed/accredited` e envio real do evento **Order (Mercado Pago)**, mas revelou que o Billing `1.9.0` aplicava a tolerância como epoch em segundos sobre o `ts` de 13 dígitos enviado em milissegundos. A correção local `1.10.1` preserva o valor recebido no manifesto HMAC e normaliza somente o cálculo da idade. A tarefa de homologação permanece aberta no [roadmap](../../ROADMAP.md) para publicar, implantar e repetir a convergência por webhook real até a entrega, sem reconciliação manual.
