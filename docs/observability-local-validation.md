...
- Logs de negócio e eventos: os três serviços emitem logs estruturados de requisições HTTP com `correlationId`; os pontos de Outbox e consumers emitem logs com `eventId`, `eventType`, `eventVersion`, `producer`, `aggregateId` e `messageStatus`, além de `topic` ou `consumer` quando aplicável.
