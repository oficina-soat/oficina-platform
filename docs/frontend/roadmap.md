...
- [ ] `[UI-FUT-USERS-TEST-001]` Cobrir contratos, autorização visual, idempotência, consistência eventual, aplicação, adapters, acessibilidade, falha parcial e fluxos E2E administrativos; homologar no `lab` com dados sentinela e investigar previamente a DLQ de `usuarioAdicionado`. Cobertura automatizada concluída localmente. No `lab`, a rodada atual confirmou criação, filtros remotos, `acoesPermitidas`, projeção sanitizada da credencial, ativação e login. O sentinela foi inativado após a interrupção controlada. A rodada final aguarda a infraestrutura publicar no API Gateway os comandos `POST /api/v1/usuarios/{usuarioId}/bloqueio` e `POST /api/v1/usuarios/{usuarioId}/reativacao`, para validar bloqueio, reativação e inativação em um único percurso ponta a ponta.
