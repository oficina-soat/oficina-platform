...
- [ ] `[D-NR-REM-005]` Executar teste de ponta a ponta no ambiente `lab` gerando uma Ordem de Serviço com caminho feliz e uma falha compensada, confirmando correlação por `correlationId` entre logs, traces, métricas e eventos. Execução e revalidação registradas em 2026-07-11 no [Relatório D-NR-REM-005 — E2E no ambiente lab](docs/d-nr-rem-005-e2e-lab-report.md): o fluxo REST passou, `Metric`, `Span` e `Log` com `correlationId` foram comprovados no New Relic para os três serviços, e a pipeline `traces/oficina-microservices` foi adicionada ao collector no `oficina-infra`. A etapa permanece pendente porque o New Relic remove o atributo literal `eventType` dos logs ingeridos; correção local preparada nos microsserviços para manter `eventType` no stdout e emitir aliases consultáveis `domainEventType` e `event.type`, faltando publicar/deployar `oficina-os-service:1.0.4`, `oficina-billing-service:1.0.5` e `oficina-execution-service:1.0.4` e reexecutar a validação remota dos eventos.
