...
- o Billing `1.10.3` persiste as instruções, cria e consulta Orders, converge atualização manual por transição condicional e Outbox determinística, não permite confirmação manual de cobrança integrada, normaliza o `ts` de 13 dígitos somente ao verificar a tolerância do webhook, classifica rejeições de assinatura sem registrar dados sensíveis e reconhece sem efeito referências assinadas desconhecidas;
...
A [homologação da continuidade do pagamento no lab](../delivery/payment-checkout-continuity-lab-evidence.md) já comprovou implantação, Quality Gates, apresentação do PIX, reconciliação server-to-server, duplicidade, ordem, concorrência, unicidade e sanitização remota. A jornada com Orders também comprovou `APRO`, order `processed/accredited` e envio real do evento **Order (Mercado Pago)**. O Billing `1.10.1` corrigiu a tolerância do `ts` de 13 dígitos; o `1.10.2` classificou um callback real histórico como `hash_mismatch`; e o `1.10.3` passou a responder `200` ao teste assinado do painel mesmo quando o ID fictício não existe localmente. A configuração atual passou pela validação HMAC nesse teste, portanto a tarefa de homologação permanece aberta no [roadmap](../../ROADMAP.md) somente até comprovar novamente a convergência por webhook real de Order até a entrega, sem reconciliação manual.
