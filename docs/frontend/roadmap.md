...
- [ ] `[UI-MVP-REM-001]` Homologar no `lab` os três fluxos do MVP com os papéis reais e registrar evidências de segurança, acessibilidade, pipeline e operação. Publicação, atendimento administrativo, composição, orçamento, pagamento, segurança, acessibilidade, pipeline e telemetria real no coletor CloudWatch estão registrados na [evidência parcial do MVP no lab](ui-mvp-lab-evidence.md). Resta executar a fila e a execução com uma credencial real exclusivamente mecânica; a conta administrativa foi corretamente impedida pelo guard visual.
