...
- o Billing `1.10.7` persiste as instruções, cria e consulta Orders, converge atualização manual por transição condicional e Outbox determinística, não permite confirmação manual de cobrança integrada, normaliza o `ts` de 13 dígitos somente ao verificar a tolerância do webhook, preserva a capitalização do `data.id`, aceita seu fallback pelo corpo quando a query não chega ao runtime, classifica rejeições de assinatura sem registrar dados sensíveis e reconhece sem efeito referências assinadas desconhecidas;
...
A [homologação da continuidade do pagamento no lab](../delivery/payment-checkout-continuity-lab-evidence.md) já comprovou implantação, Quality Gates, apresentação do PIX, reconciliação server-to-server, duplicidade, ordem, concorrência, unicidade e sanitização remota. Jornadas com Orders também comprovaram `APRO`, order `processed/accredited` e envio real do evento **Order (Mercado Pago)**. As versões `1.10.1` a `1.10.7` eliminaram divergências de timestamp, diagnóstico, referência desconhecida, origem e capitalização do `data.id`; testes assinados com o secret projetado são aceitos, mas callbacks reais de Orders continuam recebendo `401 hash_mismatch`. A hipótese residual é uma divergência entre o secret projetado e aquele usado na assinatura do modo produção. A tarefa permanece aberta no [roadmap](../../ROADMAP.md) para alinhar o secret de Webhooks do modo produção da mesma aplicação associada ao Access Token `APP_USR` e, então, comprovar a convergência por webhook real até a entrega, sem reconciliação manual.
