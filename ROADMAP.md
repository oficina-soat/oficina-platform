...
- [ ] `[B2-AUTH-USERS-REM-001]` Publicar no `lab` as versões de `[B2-AUTH-USERS-IMPL-001]`, incluindo `oficina-os-service` `1.2.1` ou superior com a correção do Quality Gate, aplicando primeiro o `oficina-infra` e depois o workflow `Deploy Lambda Lab`. Confirmar o Quality Gate remoto antes da publicação da imagem. Criar, atualizar, bloquear e inativar um usuário pelo `oficina-os-service`; confirmar os três eventos na Outbox/SNS/SQS, a projeção idempotente e ordenada no PostgreSQL da autenticação, as DLQs vazias no caminho feliz e uma falha controlada com retry. Solicitar e consumir um token de ativação uma única vez, autenticar com a nova senha e comprovar que usuários `INATIVO`, `BLOQUEADO` ou sem credencial ativada não recebem JWT.
