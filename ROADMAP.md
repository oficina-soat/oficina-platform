...
- [ ] `[D-K8S-OWNERSHIP-IMPL-001]` Transferir o ownership dos manifests Kubernetes de aplicação para os respectivos repositórios `oficina-os-service`, `oficina-billing-service` e `oficina-execution-service`, mantendo no `oficina-infra` apenas a infraestrutura compartilhada do cluster e os overlays de ambiente. Implementação local realizada em 2026-07-14: cada serviço passou a versionar `Deployment`, `Service`, `ServiceAccount`, `ConfigMap` e `kustomization.yaml` em `k8s/base/`; os workflows validam a própria base; o script do `oficina-infra` consome a base do checkout do serviço; as cópias do `oficina-infra` foram removidas; e estratégia, templates, READMEs e checklists foram atualizados. `kubectl kustomize`, `kubeconform -strict`, `actionlint`, sintaxe Bash e `diff --check` aprovaram localmente. Os `clean verify` também aprovaram 180 testes e 15 passos BDD no OS (`1.2.6`), 125 testes no Billing (`1.1.4`) e 105 testes no Execution (`1.0.19`), todos com os checks de cobertura aprovados. O item permanece aberto até os commits serem publicados e um rollout independente de cada serviço comprovar a composição no EKS, sem marcar evidência ainda inexistente.
- [ ] `[D-OBS-SAGA-IMPL-001]` Instrumentar no `oficina-os-service` as métricas `saga.instances.started.count`, `saga.instances.completed.count`, `saga.instances.compensated.count`, `saga.instances.failed.count` e `saga.step.duration`, com dimensões categóricas `service`, `sagaType`, `step` e `reason`, além dos atributos estruturados `sagaId`, `sagaStep` e estado aplicável em logs e spans. Validar testes, cobertura, cardinalidade, exposição Prometheus e ingestão no New Relic antes de concluir `[D-NR-REM-003]`.
- [ ] `[D-OBS-MP-COLLECT-IMPL-001]` Diagnosticar e corrigir por que as seis métricas `payment.provider.*`, embora implementadas no `oficina-billing-service`, não aparecem como séries `payment_provider_*` no inventário `Metric` do New Relic. Confirmar inicialização do bean, exposição em `/q/metrics`, scrape pelo collector, labels canônicas e ingestão após uma cobrança sandbox; somente então criar e validar o dashboard de `[D-NR-REM-006]`.
...
- [ ] `[FUT-SIM-OFICINA-001]` Criar no `oficina-infra` um simulador de operação cotidiana da oficina que use as APIs públicas reais para gerar tráfego sintético coerente em ambientes não produtivos. O núcleo deve ser um script reproduzível, configurável por duração, intensidade, perfil e `seed`, com distribuição ponderada semelhante à operação esperada: chegada e cadastro de clientes e veículos, abertura e evolução de ordens de serviço como fluxo predominante; movimentação de catálogo e estoque, orçamento, execução e pagamento com frequência intermediária; e cadastro, ativação, bloqueio ou inativação de usuários com baixa frequência. Incluir caminhos felizes e falhas controladas — como orçamento recusado, estoque insuficiente, pagamento recusado, retry idempotente, payload inválido e operação não autorizada — sem disparar comportamento destrutivo fora dos dados criados pelo próprio simulador. Cada ação deve emitir no console uma narrativa legível, por exemplo “Tem um cliente com um carro chegando; vamos iniciar o atendimento”, junto de timestamp, cenário, resultado HTTP, identificadores de correlação mascarados e resumo final de eventos executados, aprovados e esperadamente rejeitados. O simulador deve oferecer modo `dry-run`, limites de volume e custo, autenticação segura sem registrar tokens, marcação dos dados sintéticos, cleanup opcional, espera por efeitos assíncronos e critérios automáticos para distinguir falha esperada de regressão. Skills específicas por fluxo podem ser criadas posteriormente como interface de configuração, execução e diagnóstico, mas não devem substituir o script determinístico; um subagente simulador fica opcional para conduzir sessões exploratórias, não para o motor probabilístico ou para a evidência reproduzível.
...
O próximo passo para agentes é `[D-K8S-OWNERSHIP-IMPL-001]`: transferir os manifests Kubernetes de aplicação para os repositórios dos respectivos microsserviços, manter overlays e infraestrutura compartilhada no `oficina-infra` e preservar o deploy independente sem duplicação canônica. Itens `REM` e `EVID` ficam apartados em [Validações remotas e evidências externas](#validações-remotas-e-evidências-externas) e só devem ser tratados quando o usuário pedir explicitamente validação remota, homologação externa ou registro de evidências.

As validações remotas prioritárias restantes, quando os respectivos pré-requisitos estiverem concluídos e o ambiente externo estiver disponível, são `[D-NR-REM-003]`, `[D-NR-REM-004]`, `[D-NR-REM-006]` e os itens `EVID` finais.
