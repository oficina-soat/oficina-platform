...
- [ ] `[D-K8S-OWNERSHIP-IMPL-001]` Transferir o ownership dos manifests Kubernetes de aplicação para os respectivos repositórios `oficina-os-service`, `oficina-billing-service` e `oficina-execution-service`, mantendo no `oficina-infra` apenas a infraestrutura compartilhada do cluster e os overlays de ambiente. Cada microsserviço deve versionar sua base aplicável — no mínimo `Deployment`, `Service`, `ConfigMap` e `kustomization.yaml` ou chart Helm equivalente — e seu pipeline deve validar e aplicar essa base em conjunto com o overlay `lab`, sem depender de uma cópia divergente no `oficina-infra`. O trabalho deve atualizar a [Estratégia de entrega dos manifestos Kubernetes](docs/infrastructure/kubernetes-manifest-strategy.md), os templates, os READMEs, os workflows e scripts de deploy, preservar secrets fora do Git, validar `kubectl kustomize` e `kubeconform`, comprovar que cada serviço continua realizando rollout independente e remover somente depois as bases duplicadas do `oficina-infra`. **Este é o próximo item `IMPL` recomendado.**
