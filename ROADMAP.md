...
- [ ] `[D-JOURNEY-FRESHNESS-BILLING-PROVIDER-CONCURRENCY-IMPL-001]` Impedir que `execucaoFinalizada` e `ordemDeServicoFinalizada` façam duas solicitações concorrentes ao gateway de pagamento antes do `create-if-absent`; coordenar o ownership por orçamento também entre réplicas, preservar a retentativa quando a única solicitação ao provedor falhar e cobrir o cenário em que um concorrente recebe erro enquanto o outro conclui. A necessidade foi confirmada na [homologação bloqueada do Billing 1.7.1](docs/delivery/journey-freshness-workers-lab-evidence.md#homologação-remota-do-billing-171).
- [ ] `[D-JOURNEY-FRESHNESS-BILLING-IDEMPOTENCY-REM-001]` **Bloqueado por `[D-JOURNEY-FRESHNESS-BILLING-PROVIDER-CONCURRENCY-IMPL-001]`.** Implantar no `lab` a versão corretiva posterior ao Billing `1.7.1` e repetir a finalização concorrente pelos eventos `execucaoFinalizada` e `ordemDeServicoFinalizada`; comprovar uma única linha de pagamento, uma única Outbox, uma única solicitação concorrente ao provedor e ACK dos dois eventos sem violação de chave, retry, nova DLQ ou regressão da jornada.
