...
- [ ] `[B2-AUTH-USERS-IMPL-001]` Resolver a integração entre o CRUD administrativo de usuários do `oficina-os-service` e o store de autenticação do `oficina-auth-lambda`, sem chamada síncrona ao serviço de OS durante login e sem transportar senha nos eventos. Auditoria realizada em 2026-07-14 confirmou que a conclusão registrada anteriormente era inválida: o commit `7607693` não existe nos repositórios locais nem nos repositórios acessíveis da organização `oficina-soat`; o `oficina-auth-lambda` não contém o módulo `oficina-auth-sync-lambda` nem os endpoints de ativação; e a função remota correspondente não existe. Permanecem válidos o producer transacional de `usuarioAdicionado`, `usuarioAtualizado` e `usuarioExcluido` no `oficina-os-service`, as correções posteriores de qualidade do serviço e os tópicos, filas, DLQs e policy de consumo já declarados no `oficina-infra`. Para concluir este item, implementar o terceiro módulo Lambda com resposta parcial de lote, idempotência persistente e proteção contra snapshots fora de ordem; projetar CPF, nome, status e papéis no store PostgreSQL da autenticação; reconhecer `BLOQUEADO` no login; implementar ativação por token aleatório de 256 bits, armazenado somente como hash SHA-256, de uso único e com TTL padrão de 24 horas; integrar build, pacote e deploy independentes; declarar no Terraform todos os recursos AWS perenes ainda ausentes; e executar testes e validações locais antes de `[B2-AUTH-USERS-REM-001]`.
...
O próximo item local é `[B2-AUTH-USERS-IMPL-001]`, reaberto após a auditoria confirmar que a implementação da sincronização e da ativação não foi preservada. `[B2-AUTH-USERS-REM-001]` permanece bloqueado até a implementação, as validações locais e a publicação dos artefatos correspondentes.
