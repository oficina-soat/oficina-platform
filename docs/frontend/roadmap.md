...
- [ ] `[UI-FUT-USERS-BACKEND-001]` Evoluir OS e Auth: desacoplar publicação da Outbox do long polling que hoje adiciona até 95 segundos de latência; implementar filtros remotos por nome, CPF, estado e papel antes da paginação; `acoesPermitidas`; comandos idempotentes de bloqueio e reativação; e consulta sanitizada do estado da credencial, com autorização, Outbox e auditoria canônicas.
