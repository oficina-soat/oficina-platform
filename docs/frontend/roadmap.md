...
- [ ] `[UI-FUT-USERS-TEST-001]` Cobrir contratos, autorização visual, idempotência, consistência eventual, aplicação, adapters, acessibilidade, falha parcial e fluxos E2E administrativos; homologar no `lab` com dados sentinela e investigar previamente a DLQ de `usuarioAdicionado`. Cobertura automatizada concluída localmente; a homologação confirmou autenticação, criação, ativação, login e inativação dos sentinelas, mas permanece aberta até o `lab` receber a versão do OS que retorna `acoesPermitidas` e aplica os filtros remotos.
