...
A [homologação da continuidade do pagamento no lab](../delivery/payment-checkout-continuity-lab-evidence.md) já comprovou implantação, Quality Gates, apresentação do PIX, reconciliação server-to-server, duplicidade, ordem, concorrência, unicidade e sanitização remota. A jornada com Orders também comprovou `APRO`, order `processed/accredited` e envio real do evento **Order (Mercado Pago)**. O Billing `1.10.1` corrigiu a tolerância do `ts` de 13 dígitos e o `1.10.2` demonstrou em callback real que a rejeição residual é `hash_mismatch`. A tarefa de homologação permanece aberta no [roadmap](../../ROADMAP.md) para rotacionar o Access Token e o secret de assinatura da mesma aplicação Mercado Pago e repetir a convergência por webhook real até a entrega, sem reconciliação manual.
