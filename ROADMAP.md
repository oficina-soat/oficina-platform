...
- [ ] `[D-NR-REM-005]` Executar teste de ponta a ponta no ambiente `lab` gerando uma Ordem de Serviço com caminho feliz e uma falha compensada, confirmando correlação por `correlationId` entre logs, traces, métricas e eventos. Execução registrada em 2026-07-11 no [Relatório D-NR-REM-005 — E2E no ambiente lab](docs/d-nr-rem-005-e2e-lab-report.md): o fluxo REST passou, mas a etapa permanece pendente porque a correlação em logs, traces e eventos não foi comprovada; a falha de scrape de métricas foi corrigida no collector, mas ainda falta confirmar a ingestão em `Metric` no New Relic.
