...
- o Billing `1.10.2` persiste as instruções, cria e consulta Orders, converge atualização manual por transição condicional e Outbox determinística, não permite confirmação manual de cobrança integrada, normaliza o `ts` de 13 dígitos somente ao verificar a tolerância do webhook e classifica rejeições de assinatura sem registrar dados sensíveis;
...
A [homologação da continuidade do pagamento no lab](../delivery/payment-checkout-continuity-lab-evidence.md) já comprovou implantação, Quality Gates, apresentação do PIX, reconciliação server-to-server, duplicidade, ordem, concorrência, unicidade e sanitização remota. A jornada com Orders também comprovou `APRO`, order `processed/accredited` e envio real do evento **Order (Mercado Pago)**. O Billing `1.10.1` corrigiu a tolerância do `ts` de 13 dígitos e o `1.10.2` demonstrou em callbacks reais que a rejeição residual é `hash_mismatch`, enquanto as simulações do painel são aceitas. O próximo item do [roadmap](../../ROADMAP.md) instrumenta temporariamente os componentes não secretos da validação antes de corrigir a causa e repetir a convergência por webhook real até a entrega, sem reconciliação manual.
