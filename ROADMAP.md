...
1. `[B2-OS-USERS-IMPL-001]` Contratar e implementar CRUD REST de usuários operacionais no `oficina-os-service`.
2. `[B2-AUTH-USERS-IMPL-001]` Resolver sincronização entre usuários operacionais e autenticação serverless.
3. `[D-OBS-IMPL-003]` Instrumentar métricas de consumo Mercado Pago.
4. `[D-OBS-IMPL-004]` Instrumentar métricas de persistência, idempotência, Outbox e mensageria.
