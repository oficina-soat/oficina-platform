...
| `br.com.oficina.common.framework.web.usuario` | `oficina-os-service` | CRUD REST agregado materializado em `/api/v1/usuarios`, com UUID, autorização administrativa, status e papéis canônicos, sem as rotas redundantes `/usuarios/completos` do legado. A integração com o `oficina-auth-lambda` permanece separada. |
...
- O CRUD REST de usuários operacionais está materializado no `oficina-os-service`; sua sincronização com o `oficina-auth-lambda` permanece como pendência rastreada no [ROADMAP](../../ROADMAP.md).
