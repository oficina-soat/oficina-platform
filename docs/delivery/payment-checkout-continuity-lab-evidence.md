...
O Billing `1.10.3` foi validado pelo [PR 39](https://github.com/oficina-soat/oficina-billing-service/pull/39) e publicado pelo [run 29701919229](https://github.com/oficina-soat/oficina-billing-service/actions/runs/29701919229). O run concluiu `service-ci-validate`, SonarCloud, publicação da [release `v1.10.3`](https://github.com/oficina-soat/oficina-billing-service/releases/tag/v1.10.3) e deploy; o EKS confirmou um pod pronto com a imagem `1.10.3`.
...
O diagnóstico oficial somente leitura de notificações do Mercado Pago encontrou duas notificações de Orders, ambas com retorno `401` e nenhuma entrega bem-sucedida na janela consultada. A implementação oficial de validação do [SDK Node.js do Mercado Pago](https://github.com/mercadopago/sdk-nodejs/blob/03f66609884f724dfa718db3afba729462569a4c/src/utils/webhook/index.ts) usa o mesmo manifesto formado por `id`, `request-id` e `ts` bruto. Naquele momento, com o algoritmo, a unidade temporal e a projeção local validados, a evidência indicava secret de assinatura divergente entre a aplicação do provedor e o runtime. O teste posterior do painel, descrito a seguir, superou essa conclusão para a configuração atual.

### Teste de URL do painel e Billing `1.10.3`

Dois testes posteriores executados pelo painel do Mercado Pago, às `19:49:12Z` e `19:49:51Z`, chegaram à rota ativa `POST /api/v1/integracoes/mercado-pago/webhooks` e receberam `404` do Billing `1.10.2`. Não houve motivo de rejeição da assinatura, enquanto um probe propositalmente não assinado continuou recebendo `401`. Isso comprova que a configuração atual passou pela validação HMAC; o `404` não significava URL ausente.

O payload de teste usa `type=payment`, ação `payment.updated` e `data.id=123456`. Como essa referência fictícia não existe no Billing, a consulta local encerrava o fluxo com `ResourceNotFoundException`, convertida globalmente em `404`. O Billing `1.10.3` passou a capturar somente essa ausência após a autenticação e responder `200`, sem consultar o Mercado Pago e sem revelar se existe pagamento. Falhas transitórias continuam sem reconhecimento para preservar as retentativas do provedor.

A validação local passou com 201 testes, PostgreSQL 16, todas as 10 migrations e JaCoCo de 93,83% de linhas e 79,76% de branches. Após o deploy, uma chamada pública reproduziu o mesmo payload, sem query string e com assinatura gerada a partir do secret efetivamente projetado, e recebeu `HTTP 200`. O New Relic registrou o `POST`, a rota do webhook, `http.status=200` e `service.version=1.10.3`, sem persistir secret, assinatura ou payload. Como o ID é deliberadamente desconhecido, esse teste valida URL, assinatura e reconhecimento, mas não substitui a jornada financeira real de Order.
...
- a versão `1.10.3` registrou o teste controlado do payload do painel com `HTTP 200`, sem persistir assinatura, secret ou payload;
