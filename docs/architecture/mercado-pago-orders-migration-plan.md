...
O `lab` executa o Billing `1.10.10` no modo `orders`. A versão anterior `1.8.0` criava cobranças em `POST /v1/payments`, persistia o ID numérico do payment em `transacaoExternaId` e reconciliava por `GET /v1/payments/{id}`; essa integração apresentou corretamente o PIX, mas a cobrança sandbox permaneceu pendente na [homologação parcial](../delivery/payment-checkout-continuity-lab-evidence.md).

A versão `1.9.0` e a projeção de infraestrutura implementaram as quatro etapas preparatórias deste plano. A [evidência local da migração Orders](../delivery/payment-orders-local-evidence.md) registra contratos, migration, testes, cobertura e validações. A [homologação no lab](../delivery/payment-checkout-continuity-lab-evidence.md) comprovou criação e consulta de order, `APRO`, configuração do evento **Order (Mercado Pago)** e observabilidade. O Billing `1.10.1` corrigiu a unidade usada no cálculo da idade sem alterar o manifesto HMAC nem o contrato público; a instrumentação sanitizada até `1.10.10` comprovou que callbacks reais permanecem em `hash_mismatch`, embora simulações do painel e probes com o secret projetado sejam aceitos. O item `[D-PAYMENT-CONTINUITY-WEBHOOK-DIAG-001]` do [roadmap](../../ROADMAP.md) permanece aberto para escalonamento ao Mercado Pago, correção externa e remoção da instrumentação temporária. O [fluxo visual do diagnóstico](mercado-pago-orders-webhook-diagnostic-flow.md) explica o ponto exato da interrupção.
