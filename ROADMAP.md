...
- Auditoria de lacunas realizada em 2026-07-11 identificou que alguns itens do roadmap estavam fechados pelo contrato ou pelo schema, mas ainda sem implementação runtime real ou sem evidência remota:
  ...
  - `oficina-execution-service` recebeu acesso runtime real ao DynamoDB para catálogo, estoque, execuções, fila operacional, Outbox e idempotência em 2026-07-11, ficando pendente apenas a evidência remota no `lab` e a conexão com SNS/SQS real;
...
1. `[B2-MSG-IMPL-001]` Conectar Outbox, producers e consumers à mensageria SNS/SQS real.
2. `[B2-CONFIG-IMPL-001]` Impedir fallback silencioso para stores em memória nos profiles `prod` e `lab`.
3. `[B2-OS-USERS-IMPL-001]` Contratar e implementar CRUD REST de usuários operacionais no `oficina-os-service`.
4. `[B2-AUTH-USERS-IMPL-001]` Resolver sincronização entre usuários operacionais e autenticação serverless.
5. `[D-OBS-IMPL-003]` Instrumentar métricas de consumo Mercado Pago.
6. `[D-OBS-IMPL-004]` Instrumentar métricas de persistência, idempotência, Outbox e mensageria.
