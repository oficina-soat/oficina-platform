...
- [ ] `[D-PAYMENT-CONTINUITY-CONTRACT-001]` Evoluir contratos REST e OpenAPI para instruções PIX sanitizadas, vencimento, webhook assinado e reconciliação autenticada; separar `ATUALIZAR_STATUS` da confirmação manual em `acoesPermitidas` e definir papéis, idempotência, erros e dados proibidos em telemetria.
- [ ] `[D-PAYMENT-CONTINUITY-BILLING-IMPL-001]` Persistir e apresentar instruções PIX, validar webhook do Mercado Pago, consultar `GET /v1/payments/{id}`, reconciliar estados de forma idempotente e impedir confirmação manual de cobrança integrada sem evidência válida do provedor.
- [ ] `[D-PAYMENT-CONTINUITY-INFRA-IMPL-001]` Expor somente o webhook na borda pública, provisionar secret de assinatura, configurar notificações de teste e produção, observabilidade, alertas e runbook sem expor credenciais ou dados de pagamento.
- [ ] `[D-PAYMENT-CONTINUITY-UI-IMPL-001]` Exibir link, código PIX, vencimento e estado conforme o contrato; implementar **Atualizar situação** contra o Billing, preservar atualização manual e encaminhar para **Registrar entrega** somente após a capability canônica.
- [ ] `[D-PAYMENT-CONTINUITY-TEST-REM-001]` Cobrir contrato, segurança, webhook duplicado ou fora de ordem, reconciliação concorrente, acessibilidade e falhas; implantar e homologar no `lab` reparo → cobrança única → PIX → confirmação → entrega com sandbox do Mercado Pago e Quality Gates aprovados.
