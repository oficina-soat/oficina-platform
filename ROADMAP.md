...
- [ ] `[B2-GH-REM-001]` Confirmar proteção da branch `main` nos três repositórios de microsserviços, com PR obrigatório e checagens automáticas exigidas antes de merge. A política canônica foi documentada em [Proteção da branch main dos microsserviços](docs/delivery/github-branch-protection.md). A [verificação remota de 2026-07-13](docs/delivery/github-branch-protection-evidence.md) confirmou Rulesets ativos e PR obrigatório nos três repositórios, mas todos exigem incorretamente `develop-validate` em vez do check canônico `service-ci-validate`; a correção depende do administrador dos Rulesets e permanece fora do escopo dos agentes.
...
- [ ] `[B2-MSG-REM-001]` Validar no `lab` a mensageria real SNS/SQS da Fase 4: repetir o deploy após as correções `af4399c` e `cc30057`, confirmar a substituição das managed policies content-addressed sem `iam:CreatePolicyVersion`, criar o novo managed node group com `LabRole` antes de remover o anterior e comprovar que os três microsserviços iniciam sem negações IAM. Eventos produzidos devem sair da Outbox para o tópico SNS canônico, chegar às filas SQS dos consumidores, atualizar projeções persistidas, marcar processamento com `correlationId`, manter DLQs vazias no caminho feliz e gerar evidência de DLQ em falha controlada.
- [ ] `[B2-AUTH-USERS-REM-001]` Publicar no `lab` as versões de `[B2-AUTH-USERS-IMPL-001]`, incluindo `oficina-os-service` `1.2.2` ou superior com as correções do SonarCloud, aplicando primeiro o `oficina-infra` e depois o workflow `Deploy Lambda Lab`. Confirmar o Quality Gate remoto e zero issues antes da publicação da imagem. Criar, atualizar, bloquear e inativar um usuário pelo `oficina-os-service`; confirmar os três eventos na Outbox/SNS/SQS, a projeção idempotente e ordenada no PostgreSQL da autenticação, as DLQs vazias no caminho feliz e uma falha controlada com retry. Solicitar e consumir um token de ativação uma única vez, autenticar com a nova senha e comprovar que usuários `INATIVO`, `BLOQUEADO` ou sem credencial ativada não recebem JWT.
...
As validações remotas prioritárias restantes, quando o ambiente externo estiver disponível, são `[B2-BILL-DB-REM-001]`, `[B2-OS-DB-REM-001]`, `[B2-EXEC-DDB-REM-001]`, `[B2-IDEMP-REM-001]`, `[B2-MSG-REM-001]`, `[B2-AUTH-USERS-REM-001]`, `[B2-GH-REM-001]`, `[D-NR-REM-003]`, `[D-NR-REM-004]`, `[D-NR-REM-006]`, `[D-NR-REM-007]` e os itens `EVID` finais.
