...
- Auditoria de lacunas realizada em 2026-07-11 identificou que alguns itens do roadmap estavam fechados pelo contrato ou pelo schema, mas ainda sem implementação runtime real ou sem evidência remota:
  ...
  - o `oficina-auth-lambda` consulta um store PostgreSQL próprio de autenticação e ainda precisa de contrato explícito para provisionar credenciais e sincronizar status e papéis com o cadastro operacional de usuários.
...
O `oficina-auth-lambda` já consulta banco diretamente para autenticar CPF e senha, usando um store PostgreSQL próprio com tabelas `pessoa`, `papel`, `usuario` e `usuario_papel`. Ele não consulta o `oficina-os-service` por REST no caminho de login. A recomendação inicial é manter o caminho crítico de login independente de chamada síncrona ao `oficina-os-service`, resolvendo a consistência por provisionamento explícito de credenciais e sincronização de status e papéis. O item `[B2-AUTH-USERS-IMPL-001]` deve definir o bootstrap seguro da credencial sem transportar senha pelo OS e alinhar o estado `BLOQUEADO`, ainda ausente no modelo da Lambda. Se a decisão futura for fazer a Lambda consultar o serviço de OS durante o login, a [ADR-003 - Serverless para Autenticação e Notificações](adr/ADR-003%20-%20Serverless%20para%20Autenticação%20e%20Notificações.md), o contrato REST, o OpenAPI e os runbooks de disponibilidade devem ser atualizados antes da implementação.
...
- [ ] `[B2-AUTH-USERS-IMPL-001]` Resolver a integração entre o CRUD administrativo de usuários do `oficina-os-service` e o store de autenticação do `oficina-auth-lambda`. Estado atual verificado: a Lambda consulta seu próprio PostgreSQL de autenticação, não o serviço de OS, e seu modelo ainda não reconhece `BLOQUEADO`. A recomendação é evitar chamada síncrona ao `oficina-os-service` durante login, definir provisionamento seguro de credenciais sem transportar senha pelo OS e sincronizar status e papéis; se a alternativa escolhida for consulta síncrona ao serviço, atualizar a [ADR-003 - Serverless para Autenticação e Notificações](adr/ADR-003%20-%20Serverless%20para%20Autenticação%20e%20Notificações.md), os contratos e os runbooks antes de implementar.
...
1. `[B2-AUTH-USERS-IMPL-001]` Provisionar credenciais e sincronizar status e papéis entre usuários operacionais e autenticação serverless.
2. `[D-OBS-IMPL-003]` Instrumentar métricas de consumo Mercado Pago.
3. `[D-OBS-IMPL-004]` Instrumentar métricas de persistência, idempotência, Outbox e mensageria.
