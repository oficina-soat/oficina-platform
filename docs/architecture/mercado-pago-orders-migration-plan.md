...
A versão `1.9.0` e a projeção de infraestrutura implementaram as quatro etapas preparatórias deste plano. A [evidência local da migração Orders](../delivery/payment-orders-local-evidence.md) registra contratos, migration, testes, cobertura e validações. A [homologação no lab](../delivery/payment-checkout-continuity-lab-evidence.md) comprovou criação e consulta de order, `APRO`, configuração do evento **Order (Mercado Pago)** e observabilidade. O Billing `1.10.1` corrigiu a unidade usada no cálculo da idade sem alterar o manifesto HMAC nem o contrato público; o `1.10.2`, já publicado e implantado, classificou o callback real residual como `hash_mismatch`. A etapa remota `[D-PAYMENT-CONTINUITY-TEST-REM-001]` permanece aberta para rotacionar as credenciais da mesma aplicação Mercado Pago e repetir o webhook real até a entrega.
