...
- [ ] `[UI-OS-ITEMS-DISCOVERY-001]` Auditar o fluxo atual, os estados da OS, os catálogos de serviços e peças, os eventos e os consumidores para definir ownership, momento de inclusão, quantidade, remoção, alteração e reflexos em orçamento e estoque, registrando a decisão em ADR quando houver mudança arquitetural.
- [ ] `[UI-OS-ITEMS-CONTRACT-001]` Evoluir as OpenAPI de Execution e OS para oferecer catálogos paginados e filtráveis e a composição detalhada da OS, com identificadores, quantidades, snapshots necessários, erros, idempotência e `acoesPermitidas` para incluir, alterar e remover itens.
- [ ] `[UI-OS-ITEMS-CATALOG-001]` Ajustar no `oficina-execution-service` somente as lacunas de consulta dos catálogos identificadas na auditoria, incluindo autorização, filtros, paginação e indicação canônica de disponibilidade, sem expor decisão de estoque ao frontend.
- [ ] `[UI-OS-ITEMS-BACKEND-001]` Implementar no `oficina-os-service` a persistência e os comandos idempotentes de composição da OS, revalidando estado, autorização, item, quantidade e ações permitidas exclusivamente no backend.
- [ ] `[UI-OS-ITEMS-INTEGRATION-001]` Integrar a composição com eventos, Billing, Execution e estoque, preservando snapshots financeiros, deduplicação, consistência eventual, compensação e observabilidade por `correlationId`.
...
- [ ] `[UI-OS-ITEMS-VIEW-001]` Implementar no detalhe da OS a pesquisa e seleção acessível de serviços e peças, quantidade, composição atual e confirmações, exibindo inclusão, alteração e remoção somente quando informadas por `acoesPermitidas`.
